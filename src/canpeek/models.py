...
def _get_all_decodings(
    frame: CANFrame,
    dbc_databases: List[cantools.db.Database],
    pdo_databases: List[cantools.db.Database],
    canopen_enabled: bool,
) -> List[str]:
    """Helper function to get all possible decodings for a given frame."""

    decoded_parts = []

    # 1. Try regular DBC databases
    ...
    # 3. Fallback to generic CANopen decoding
    if canopen_enabled:
